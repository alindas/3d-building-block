...
/**
 * 此插件官方已不维护，存在一些 THREE API 过时导致的问题
 * 1. BufferGeometry.addAttribute() 为 BufferGeometry.setAttribute()
 * 2. 弃用 Matrix*.applyToBufferAttribute 方法：https://github.com/mrdoob/three.js/issues/18006
 * 3. MathUtils 代替 Math
 */
...
module.exports = (function () {
  ...
  function parseParameters(FBXTree, properties, textureMap, ID, connections) {
    ...
    if (properties.Emissive) {
      parameters.emissive = new THREE.Color().fromArray(
        properties.Emissive.value,
      );
    } else if (
      properties.EmissiveColor &&
      properties.EmissiveColor.type === 'Color'
    ) {
      // The blender exporter exports emissive color here instead of in properties.Emissive
      parameters.emissive = new THREE.Color().fromArray(
        properties.EmissiveColor.value,
      );
    }
    ...
    connections.get(ID).children.forEach(function (child) {
      ...
      switch (type) {
        ...
        case 'DisplacementColor':
          parameters.displacementMap = getTexture(
            FBXTree,
            textureMap,
            child.ID,
            connections,
        ...
        case 'EmissiveColor':
          parameters.emissiveMap = getTexture(
            FBXTree,
            textureMap,
            child.ID,
            connections,
        ...
        case 'NormalMap':
          parameters.normalMap = getTexture(
            FBXTree,
            textureMap,
            child.ID,
            connections,
        ...
        case 'SpecularColor':
          parameters.specularMap = getTexture(
            FBXTree,
            textureMap,
            child.ID,
            connections,
        ...
        case 'TransparentColor':
          parameters.alphaMap = getTexture(
            FBXTree,
            textureMap,
            child.ID,
            connections,
  ...
  function parseMeshGeometry(FBXTree, relationships, geoNode, deformers) {
    ...
    if ('GeometricRotation' in modelNode) {
      ...
      var array = modelNode.GeometricRotation.value.map(
    ...
    if ('GeometricTranslation' in modelNode) {
      preTransform.setPosition(
        new THREE.Vector3().fromArray(modelNode.GeometricTranslation.value),
      );
    }

    if ('GeometricScaling' in modelNode) {
      preTransform.scale(
        new THREE.Vector3().fromArray(modelNode.GeometricScaling.value),
      );
    }
  ...
  function setModelTransforms(FBXTree, model, modelNode) {
    ...
    if ('Lcl_Rotation' in modelNode) {
      var rotation = modelNode.Lcl_Rotation.value.map(THREE.MathUtils.degToRad);
      rotation.push('ZYX');
    ...
    if ('PreRotation' in modelNode) {
      var array = modelNode.PreRotation.value.map(THREE.MathUtils.degToRad);
      array[3] = 'ZYX';
          ...
          if (curveNodesMap.has(child.ID)) {
            ...
            if (
              ...
              if (layerCurveNodes[i] === undefined) {
                ...
                if ('Lcl_Translation' in rawModel)
                  node.initialPosition = rawModel.Lcl_Translation.value;

                if ('Lcl_Rotation' in rawModel)
                  node.initialRotation = rawModel.Lcl_Rotation.value;

                if ('Lcl_Scaling' in rawModel)
                  node.initialScale = rawModel.Lcl_Scaling.value;
  ...
  function generateTracks(rawTracks) {
    ...
    if (
      ...
      var positionTrack = generateVectorTrack(
        rawTracks.modelName,
        rawTracks.T.curves,
        rawTracks.initialPosition,
        'position',
      );
    ...
    if (
      ...
      var scaleTrack = generateVectorTrack(
        rawTracks.modelName,
        rawTracks.S.curves,
        rawTracks.initialScale,
        'scale',
      );
  ...
  function generateRotationTrack(
    modelName,
    curves,
    initialValue,
    preRotations,
