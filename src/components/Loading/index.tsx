...
export default function Loading(props: ILoading) {
  ...
  return (
    <div className="text-center" hidden={!visible}>
      {type == 2 ? (
        <span className="loading-tips conic">{msg}</span>
      ) : (
        <span className="loading-tips">
          {type == 0 ? (
            ...
            <span className="upload-icons">
              <svg
                className="loading-anim-upload-upper"
              ...
              <svg
                className="loading-anim-upload-lower"
                viewBox="64 64 896 896"
                focusable="false"
                data-icon="cloud-upload"
                width="1em"
...
Loading.closer = {} as { [id: number]: TPayload };
...
Loading.start = (config?: TLoading) => {
  ...
  ReactDOM.render(<Loading id={originId} {...config} />, mount);
  return originId;
};

Loading.refresh = (id?: number, msg?: string | React.ReactNode) => {
  if (id) {
    Loading.closer[id].msg = msg;
  } else {
    Loading.closer[originId].msg = msg;
  }
};

Loading.done = (id?: number) => {
  let key = id ?? originId;
  let c = Loading.closer[key];
  c && c.closer();
  delete Loading.closer[key];
};
